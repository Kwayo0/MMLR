...
func _on_warp_zone_body_entered(_body: Node3D) -> void:
	var three = preload("res://scenes/main_sub_gate_3.tscn").instantiate()
	main.add_child(three)
	three.get_node("Megaman").position = Vector3(-7,-2,-16.5)
...
func _on_warp_zone_2_body_entered(_body: Node3D) -> void:
	var four = preload("res://scenes/main_sub_gate_4.tscn").instantiate()
	main.add_child(four)
...
func _on_warp_zone_3_body_entered(_body: Node3D) -> void:
	var one = load("res://scenes/main_sub_gate_1.tscn").instantiate()
	main.add_child(one)
